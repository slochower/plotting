...
def prepare_plot():
    ...
    mpl.rcParams['text.latex.preamble'] = [
        r'\usepackage{amsmath}', r'\usepackage{helvet}',
        r'\usepackage[EULERGREEK]{sansmath}', r'\sansmath',
        r'\renewcommand{\familydefault}{\sfdefault}',
        r'\usepackage[T1]{fontenc}', r'\usepackage{graphicx}'
    ]
...
def setup_plot(y_label, x_label, axis_padding=0.06):
    ...
    y_label_y = y + h / 2.0
    ax.set_ylabel(
        y_label, verticalalignment='top', horizontalalignment='center')
...
def panel_label(label=None, panel_xoffset=-0.24, panel_yoffset=0.95):
    """
    Add a small label to the top left region of a figure.
    """
    ax = plt.gca()
    ax.annotate(
        r'\textbf{{ {} }}'.format(label),
        xy=(0, 0),
        xytext=(panel_xoffset, panel_yoffset),
        xycoords='axes fraction',
        fontsize=24,
